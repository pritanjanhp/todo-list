        ...
        <div className='todo-list'>
            {
                todos.length !== 0 ? todos.map((item, idx) => (
                    updateData.id === item.id ? <form className='todo-item' onSubmit={() => handleUpdate(updateData.id)}>
                        <input type='text' placeholder='update' value={updateData.text} onChange={(e) => setUpdateData({ ...updateData, text: e.target.value })} />
